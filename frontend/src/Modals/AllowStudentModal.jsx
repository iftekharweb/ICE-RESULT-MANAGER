...
const AllowStudentModal = ({ handleAllow, semester_id, form_id }) => {
  ...
  const teacherData = async () => {
    try {
      const res = await axios.get(
        `${import.meta.env.VITE_BASEURL}/teachers/${teacher_id}/`
      );
      if (res.data) {
        setInfo(res.data);
    ...
    <>
      {!adding && (
        <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-sm flex justify-center items-center">
          <div className="bg-white py-5 px-8 rounded-xl w-[60%]">
            <div>
            ...
            <div className="pb-5 pt-3">
              {info?.assigned_sections.map((section) => (
                section?.course?.semester?.id === semester_id && <article className="rounded-lg border border-gray-100 bg-white p-4 shadow-sm transition hover:shadow-lg sm:p-6">
                  ...
                  <a
                    href="#"
                    className="group mt-4 inline-flex items-center gap-1 text-sm font-medium text-[#03C9D7] hover:text-green-600"
                  >
                    Add students
                    <span
                      aria-hidden="true"
...
export default AllowStudentModal;
