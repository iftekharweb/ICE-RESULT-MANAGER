...
const StudentFormFillUpModal = ({ handleFill, semester_id, form_id }) => {
  ...
  const get_student_id = async () => {
    const res = await actions.get_student_id(authUserId, authToken);
    if (!res.error) {
  ...
  const makeFormed = async (id) => {
    const postData = { is_formed: true };
    const res = await actions.complete_form_fill_up_by_student(
      postData,
      id,
      authToken
    );
                        ...
                        <div>
                          {course.all_ok === true &&
                            course.is_completed === false && (
                              <button
                                className="border rounded-md border-sky-500 hover:bg-sky-500 px-2 py-1"
                                onClick={() => {
                                  changeInfo(
                                    course.is_completed,
                                    course.sections
